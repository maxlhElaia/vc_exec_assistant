...
def initialize_state():  
    ...
    for company in companies:  
        ...
        avg_salary_per_staff = random.uniform(4000, 8000)  # Average monthly salary per staff  
        other_expenses_percentage = random.uniform(0.1, 0.2)  # Other expenses as a percentage of revenue  
...
def generate_next_month(states, data):  
    ...
    for company_name, state in states.items():  
        ...
        staff_growth_rate = net_growth_rate * 0.5  # Staff grows at half the rate of revenue  
        staff = max(1, int(staff * (1 + staff_growth_rate)))  
  
        # Update expenses  
        avg_salary_per_staff *= 1 + random.uniform(0.0, 0.02)  # Salaries may increase slightly over time  
        salary_expenses = staff * avg_salary_per_staff  
        other_expenses = other_expenses_percentage * revenue  # Other expenses proportional to revenue  
        operating_expenses = salary_expenses + other_expenses  
        ebitda = revenue - operating_expenses  
        ...
        if runway <= 3 and cash > 0:  
            if random.random() <= 0.5:  
                ...
                state['growth_boost_months'] = growth_boost_months  
                print(f"{company_name} raised ${funding_amount:,.2f} in funding on {date.strftime('%Y-%m')}.")  
            else:  
                # Company fails to raise funds and goes defunct  
                print(f"{company_name} failed to raise funds and is now defunct on {date.strftime('%Y-%m')}.")  
                state['defunct'] = True  
                continue  # Skip adding data for this month  
  
        # Check if the company has run out of cash  
        if cash <= 0:  
            print(f"{company_name} has run out of cash and is now defunct on {date.strftime('%Y-%m')}.")  
            state['defunct'] = True  
            continue  # Skip adding data for this month  
...
def generate_reporting_change_signals(states, data):  
    ...
    for company_name, records in data.items():  
        if len(records) >= 2:  
            ...
            latest_runway = float('inf') if latest_record['runway_months'] == 'infinite' else latest_record['runway_months']  
            previous_runway = float('inf') if previous_record['runway_months'] == 'infinite' else previous_record['runway_months']  
